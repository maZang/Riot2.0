...
for comp, stats in data.items():
	...
	for champs in comp:
		comp.remove(champs)
		for champ in comp:
			if champ not in champ_syn_dict[champs]:
				champ_syn_dict[champs][champ] = stats
				champ_syn_dict[champ][champs] = stats
			else:
				win, game = stats
				prev_win, prev_game = champ_syn_dict[champs][champ]
				champ_syn_dict[champs][champ] = (win + prev_win, game + prev_game)
...
for champ in new_syn_dict:
	...
	for member in new_syn_dict[champ]:
		...
		win_test, game_test = new_syn_dict[member][champ]
		if win != win_test:
			print("Error")
		if game != game_test:
			print("Error game")
